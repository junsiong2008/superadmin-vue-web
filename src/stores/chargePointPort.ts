...
export const useChargePointPortStore = defineStore('chargePointPort', () => {
  ...
  const get = (payload: Payload): Promise<void | AxiosResponse> => {
    return axios
      .get(`${url}/charge-point-port/${payload.path}`, {
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/x-www-form-urlencoded',
          token: localStorage.getItem('evhome.superadmin.access_token')
  ...
  const add = (payload: Payload): Promise<void | AxiosResponse> => {
    return axios
      .post(`${url}/charge-point-port${payload.path ? `/${payload.path}` : ''}`, payload.params, {
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/x-www-form-urlencoded',
          token: localStorage.getItem('evhome.superadmin.access_token')
  ...
  const update = (payload: Payload): Promise<void | AxiosResponse> => {
    return axios
      .put(`${url}/charge-point-port${payload.path ? `/${payload.path}` : ''}`, payload.params, {
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/x-www-form-urlencoded',
          token: localStorage.getItem('evhome.superadmin.access_token')
