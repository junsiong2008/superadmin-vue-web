...
export const useUserGroupStore = defineStore('userGroup', () => {
  ...
  const getAll = (payload: Payload): Promise<void | AxiosResponse> => {
    return axios
      .get(`${url}/user-group?${payload.query}`, {
        headers: {
  ...
  const get = (payload: Payload): Promise<void | AxiosResponse> => {
    return axios
      .get(`${url}/user-group/${payload.path}`, {
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/x-www-form-urlencoded',
          token: localStorage.getItem('evhome.superadmin.access_token')
  ...
  const add = (payload: Payload): Promise<void | AxiosResponse> => {
    return axios
      .post(`${url}/user-group`, payload.params, {
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/x-www-form-urlencoded',
          token: localStorage.getItem('evhome.superadmin.access_token')
  ...
  const update = (payload: Payload): Promise<void | AxiosResponse> => {
    return axios
      .put(`${url}/user-group${payload.path ? `/${payload.path}` : ''}`, payload.params, {
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/x-www-form-urlencoded',
          token: localStorage.getItem('evhome.superadmin.access_token')
